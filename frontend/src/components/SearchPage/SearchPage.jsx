...
function SearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const geoRes = await camerasAPI.resolveGeo(searchQuery);
      const { latitude, longitude } = geoRes.data;
      // Затем ищем парковки
      const parkRes = await camerasAPI.searchParking({ lat: latitude, lon: longitude });
      // Преобразуем результат в список адресов (временное решение)
      setAddresses([searchQuery]);
    } catch (error) {
  ...
  return (
    <div className={styles['mobile-container']}>
      ...
      <div className={styles['search-section']}>
        ...
        <div className={styles['glass-round']} onClick={handleSearch} style={{ cursor: 'pointer' }}>
